...
async def test_can_use_request_scope_dependency_async(make_app, make_asgi_client):
    ...
    @app.get("/")
    async def root(dep: int = Provide(get_42, wrap=True)):
        return {"dep": dep}
...
async def test_can_use_request_scope_dependency_sync(make_app, make_asgi_client):
    ...
    @app.get("/")
    def root(dep: int = Provide(get_42, wrap=True)):
        return {"dep": dep}
...
async def test_can_use_request_scope_dependency_async_and_inject_decorator(
    make_app, make_asgi_client
):
    init_counter = 0
    closing_counter = 0
    ...
    @app.get("/")
    @inject
    async def root(dep: int = Depends(Provide(get_42))):
        return {"dep": dep}
...
async def test_can_use_request_scope_dependency_sync_and_inject_decorator(
    make_app, make_asgi_client
):
    init_counter = 0
    closing_counter = 0
    ...
    @app.get("/")
    @inject
    def root(dep: int = Depends(Provide(get_42))):
        return {"dep": dep}
